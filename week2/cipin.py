...
# 8. 特征词筛选
# 相关博客 https://zhuanlan.zhihu.com/p/187580421
# 相关博客 https://blog.csdn.net/qq_38614074/article/details/136532110
# 8.1 利用词频来进行特征词的筛选
def filter_words_by_frequency(word_freq, threshold):
    return [word for word, freq in word_freq.items() if freq >= threshold]

# 8.2 通过特征词来对文本进行向量表示
def vectorize_text(text, filtered_words):
    vector = [1 if word in text else 0 for word in filtered_words]
    return vector
...
def main():
    ...
    # 8. 特征词筛选和向量表示
    # 8.1 利用词频来进行特征词的筛选
    filtered_words = filter_words_by_frequency(word_freq, 2000)
    print("\n词频大于等于2000的词：", filtered_words)
    
    # 8.2 通过特征词来对文本进行向量表示，并计算相似度
    # 选择两个示例句子
    if len(lines) >= 2:
        sentence1 = lines[27]
        sentence2 = lines[28]
        ...
        # 将句子转换为向量
        vector1 = vectorize_text(sentence1, filtered_words)
        vector2 = vectorize_text(sentence2, filtered_words)
